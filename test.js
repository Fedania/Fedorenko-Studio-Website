document.addEventListener("DOMContentLoaded", function () {
    fetch("../data/projects.json")
        ...
        .then(data => {
            const projects = data.project; // Access the correct property in JSON
    ...
    function renderCategory(category, projects, container, showMore) {
        ...
        section.innerHTML = `
            ...
            <div class="projects-grid">
                ${projects.map(p => `<img class="project" src="${p.thumbnail}" alt="${p.title}">`).join("")}
            </div>
            ${showMore ? `<a class="see-more" href="?category=${category}">See More</a>` : ""}
        `;
